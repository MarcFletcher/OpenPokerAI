...
extern "C" __global__ void Evaluate_Hand(int *handEvalData, float *randoms, int hand1, int hand2, int table1, int table2, int table3, int table4, int table5, int numberPlayers, int *totalResult)
{
	...
	int numberCardsRequired = 2 * numberPlayers - 45 + index;
	int numberCardsInDeck = index;

	for(int i = numberCardsInDeck - 1; i >= numberCardsInDeck - numberCardsRequired; i--)
	{
	...
	if(table1 == 0)
	{
		table1 = randomNumbers[threadIdx.x][threadIdx.y][numberCardsInDeck - randomIndex - 1] + 1;			
		randomIndex++;
	}
	if(table2 == 0)
	{
		table2 = randomNumbers[threadIdx.x][threadIdx.y][numberCardsInDeck - randomIndex - 1] + 1;			
		randomIndex++;
	}
	if(table3 == 0)
	{
		table3 = randomNumbers[threadIdx.x][threadIdx.y][numberCardsInDeck - randomIndex - 1] + 1;			
		randomIndex++;
	}*/
	if(table4 == 0)
	{
		table4 = randomNumbers[threadIdx.x][threadIdx.y][numberCardsInDeck - randomIndex - 1] + 1;			
		randomIndex++;
	}
	if(table5 == 0)
	{
		table5 = randomNumbers[threadIdx.x][threadIdx.y][numberCardsInDeck - randomIndex - 1] + 1;			
		randomIndex++;
	}
